  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      map.current.addLayer({
        ...
        layout: {
          visibility: "visible", // This will be toggled by the zoom handler
        },
        paint: {
      ...
      map.current.on("zoom", () => {
        ...
        const allPolygons = [
          "sixteen-projects-area",
          "seven-projects-area",
          "two-projects-area",
          "zakhir-police-station-area",
          "muraba-area",
          "zero-projects-area",
          "police-admin-villas-area",
          "alamerah",
        ]

        allPolygons.forEach((id) => {
          if (map.current) {
            // Set visibility of both fill and outline layers
            const visibility = zoom >= ZOOM_THRESHOLD ? "none" : "visible"
            map.current.setLayoutProperty(`${id}-fill`, "visibility", visibility)
            map.current.setLayoutProperty(`${id}-outline`, "visibility", visibility)
          }
        })
      })
